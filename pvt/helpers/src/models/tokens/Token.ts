...
export default class Token {
  ...
  static async deployedAt(address: Account): Promise<Token> {
    const instance = await deployedAt('v2-solidity-helpers/TestToken', TypesConverter.toAddress(address));
    ...
    if (symbol === 'WETH') {
      return new Token(
        ...
        symbol,
        decimals,
        await deployedAt('v2-solidity-helpers/TestWETH', TypesConverter.toAddress(address))
      );
    }
